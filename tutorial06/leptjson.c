...
static int lept_parse_object(lept_context* c, lept_value* v) {
    ...
    for (;;) {
		...
		/*c->json++;
		这里是个bug，很久才找到。进行自增操作多此一举，因为接下来的lept_parse_string_raw()操作
		会进行EXPECT('\"')操作。*/
    ...
    switch (v->type) {
		...
		case LEPT_OBJECT:
			for (i = 0; i < v->u.o.size; i++) {
				free(v->u.o.m[i].k);//删除键，即字符串
				lept_free(&v->u.o.m[i].v);//删除值，即lept_value v
			}
			free(v->u.o.m);//删除object数组，其内存来内需
        default: break;
    }
    v->type = LEPT_NULL;
}
